...
class PublicController extends Controller
{
    ...
    public function postSubscribe(NewsletterRequest $request, BaseHttpResponse $response)
    {
		...
		$apiURL = 'https://awacmail.com/api/v1/subscribers';

        
		$postInput = [
            'list_uid' => '6315df591570b',
            'api_token' => 'oq8VqSpxHQ0P41jRkZWluigeI3aLC7FPr0usaPJwFgD9jqGkqyJvNoldr2CZ',
        ...
        $response1 = Http::withHeaders($headers)->post($apiURL, $postInput);
  
        $statusCode = $response1->status();
        $responseBody = json_decode($response1->getBody(), true);
